...
import { getSmartReminderType, prayerTimes } from "../../data/dhikr";
import { getTraditionById } from "../../data/traditions";
import { surahs } from "../../data/quran";
import { useDatabase } from "../../lib/useDatabase";
import { getActiveDhikrItems, loadUserData } from "../../lib/userData";
        ...
        <GlowCard className="p-5" gold>
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p style={{ color: "#CF9555", fontSize: "0.85rem" }}>
                        at {next.time}
                    </p>
                </div>
...
function SmartReminderBanner() {
    ...
    const Icon =
        reminder.type === "morning"
            ? Sun
            : reminder.type === "evening"
              ? Moon
              : Star;
    ...
    return (
        <motion.div
            ...
            style={{
                background:
                    "linear-gradient(135deg, rgba(220,160,72,0.2) 0%, rgba(106,36,40,0.6) 100%)",
        ...
        >
            <div className="p-4 flex items-center gap-4">
                <motion.div
                    ...
                    style={{
                        background: "rgba(220,160,72,0.2)",
                        border: "1px solid rgba(220,160,72,0.4)",
                    }}
                >
                ...
                <div className="flex-1">
                    ...
                    <p style={{ color: "#D7C29F", fontSize: "0.78rem" }}>
    ...
    return (
        <div className="relative w-36 h-36 mx-auto">
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center">
                ...
                <span style={{ color: "#D7C29F", fontSize: "0.7rem" }}>
                    of {total}
                </span>
                <span
                    style={{
        ...
        >
            <div className="p-4 flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                    ...
                    <p style={{ color: "#D7C29F", fontSize: "0.75rem" }}>
                        {notif.subtitle}
                    </p>
                </div>
...
export function Home() {
    const [notifs, setNotifs] = useState<NotifCard[]>(emptyNotifs);
    const [completedDhikr, setCompletedDhikr] = useState(0);
    ...
    const userData = useMemo(() => loadUserData(), []);
    const userId: string = userData.id || userData.userId || "default";
    const activeDhikrItems = useMemo(
        () => getActiveDhikrItems(userData),
        [userData],
    );
    const activeDhikrIds = useMemo(
        () => new Set(activeDhikrItems.map((item) => item.id)),
        [activeDhikrItems],
    );
    const totalDhikr = activeDhikrItems.length;
    const featuredDhikr = useMemo(
        () =>
            activeDhikrItems.find((item) => item.category === "morning") ??
            activeDhikrItems[0] ??
            null,
        [activeDhikrItems],
    );
    const activeTradition = getTraditionById(userData.traditionId);
    const pinnedWird =
        userData.customWirds?.find((w) => w.id === userData.pinnedWirdId) ||
        null;
    const progressSourceLabel =
        userData.dhikrMode === "custom" && pinnedWird
            ? `Pinned Wird: ${pinnedWird.name}`
            : "Basic Adhkar";
    ...
    // Load progress + build "recent notifications" from DB
    useEffect(() => {
        let cancelled = false;
        ...
        (async () => {
            try {
                ...
                const todayRows = await currentDb.getDailyAdhkarProgress(
                    userId,
                    todayKey,
                );
                if (cancelled) return;

                // Daily progress = how many adhkar items have count>0 today
                setCompletedDhikr(
                    (todayRows || []).filter((r: any) => {
                        const rowId = r.adhkarId ?? r.adhkar_id;
                        return (r.count ?? 0) > 0 && activeDhikrIds.has(rowId);
                    }).length,
                );

                // This week = sum counts over last 7 days
                let weekTotal = 0;
                for (let i = 0; i < 7; i++) {
                ...
                // Total dhikr = bounded sum over last 365 days
                let total = 0;
                for (let i = 0; i < 365; i++) {
                    const d = new Date(today);
                    d.setDate(d.getDate() - i);
                ...
                setTotalDhikrCount(total);

                // Streak: consecutive days (from today backward) with any count>0
                let streak = 0;
                for (let i = 0; i < 365; i++) {
                    const d = new Date(today);
                    d.setDate(d.getDate() - i);
                ...
                // Recent notifications = from local DB notifications table
                try {
                    ...
                    const recent = rows.map((r: any): NotifCard => {
                        const when = r.createdAt
                            ? new Date(r.createdAt * 1000)
                            : new Date();
                        return {
                            id: r.id,
                            type: (r.notifType || "info") as any,
                            title: r.title,
                            subtitle: r.subtitle || when.toLocaleString(),
                        };
                    });
                    setNotifs(recent);
                } catch {
                ...
                // If DB isn't ready yet, keep zeros.
            }
        })();
    ...
    const stats = useMemo(
        () => [
            ...
            {
                label: "Total Dhikr",
                value: String(totalDhikrCount),
                icon: Award,
                color: "#48B16E",
            },
        ],
        [dayStreak, thisWeek, totalDhikrCount],
    );
                ...
                <GlowCard className="overflow-hidden">
                    <div className="p-5 space-y-4">
                        ...
                        <div className="space-y-2">
                            ...
                            >
                                {featuredDhikr?.arabic ||
                                    "No active dhikr item selected."}
                            </p>
                            <p
                                style={{
                                    color: "#CF9555",
                            ...
                            >
                                {featuredDhikr?.transliteration ||
                                    "Enable modules in settings to see your daily snippet."}
                            </p>
                            <p
                                style={{
                                    color: "#D7C29F",
                            ...
                            >
                                "
                                {featuredDhikr?.translation ||
                                    "Your personalized dhikr appears here."}
                                "
                            </p>
                            <p
                                style={{
                            ...
                            >
                                {featuredDhikr?.source ||
                                    activeTradition.primarySource}
                            </p>
                        </div>
            ...
            >
                <div className="flex items-center justify-between mb-3">
                    ...
                    <button
                        ...
                        style={{ color: "#DCA048", fontSize: "0.8rem" }}
                    >
                        View all
                    </button>
                </div>
