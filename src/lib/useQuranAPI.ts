...
async function getSurahFromCache(
    surahNumber: number,
    ...
    try {
        ...
        return new Promise((resolve) => {
            const request = db
                .transaction(SURAHS_STORE, "readonly")
...
async function getCacheMetadata(): Promise<any> {
    try {
        ...
        return new Promise((resolve) => {
            const request = db
                .transaction(CACHE_METADATA_STORE, "readonly")
...
export function useQuranAPI(): UseQuranAPI {
    ...
    const getSurah = useCallback(
        async (
            surahNumber: number,
            edition: string = DEFAULT_EDITIONS.text,
        ): Promise<SurahData> => {
            ...
            try {
                ...
                const errorObj: QuranApiError = {
                    message:
                        err.message || `Failed to fetch surah ${surahNumber}`,
    ...
    const getVerse = useCallback(
        async (surahNumber: number, verseNumber: number): Promise<Verse> => {
            try {
                ...
                const verse = surah.ayahs.find(
                    (v) => v.numberInSurah === verseNumber,
                );

                if (!verse) {
                    throw new Error(
                        `Verse ${verseNumber} not found in Surah ${surahNumber}`,
                    );
                }
                ...
                const errorObj: QuranApiError = {
                    message:
                        err.message ||
                        `Failed to fetch verse ${surahNumber}:${verseNumber}`,
