...
import { morningAdhkar, eveningAdhkar, afterPrayerAdhkar, generalDhikr } from '../data/dhikr';
...
export function getNotificationConfig(): NotificationConfig {
  try {
    const userData = JSON.parse(localStorage.getItem('dhikr_user_data') || '{}');
    return {
      enabled: true,
      quietStart: userData.quietStart || '22:00',
      quietEnd: userData.quietEnd || '06:00',
      triggers: userData.reminderTriggers || ['prayer-based', 'morning', 'evening'],
      soundEnabled: true,
    };
  } catch {
    return {
...
export function isQuietHours(): boolean {
  ...
  const [quietStartHour, quietStartMin] = config.quietStart.split(':').map(Number);
...
export function getRandomAdhkar() {
  const allAdhkar = [...morningAdhkar, ...eveningAdhkar, ...afterPrayerAdhkar, ...generalDhikr];
 ...
 */
export function createAdhkarNotification(adhkar: any, trigger: string): AdhkarNotification {
  return {
...
export function getActivityStats() {
  try {
    const stats = JSON.parse(localStorage.getItem('dhikr_activity_stats') || '{}');
    return {
